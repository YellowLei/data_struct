//练习http://acm.hust.edu.cn/vjudge/contest/view.action?cid=17321#problem/C
			...
			for(int i=0;i<Max;i++)
			{
				...
				else 	//创建fail指针
				{
					...
					while(temp!=NULL)
					{
						...
						temp=temp->fail;  //向上寻找直到根节点的fail指针是NULL
					}
					if(temp==NULL)
						tmp->next[i]->fail=root;
				}
...
//	重复这2个过程中的任意一个，直到模式串走到结尾为止。
	int query(char *s)
	{
		...
		for(int i=0;s[i];i++)
		{
			...
			{
				ans+=temp->ncount;	//找到满足的，将ncount加到ans中
				temp->ncount=-1;		//将ncount置为-1，防止重复计数
